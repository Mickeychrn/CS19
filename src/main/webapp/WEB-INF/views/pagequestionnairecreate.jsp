...
<script>
	function rangeClick() {
		...
		if (!isNum) {//if it is not a number, alert
			alert("Please ensure the number of description is number");
			return;
		}
		var box = document.getElementById('box');
	...
	function submit() {
		...
		if(isRealNum(minvalue) == false || isRealNum(maxvalue) == false){
			alert("Please ensure the min value and max value is number")
			return;
		}
		...
		if (isEmpty) {
			alert("Please fill all the descriptions")
			return;
		}
		var descriptionnumber = psyArr.length;
